...
Build a production-oriented **AI Memory Gateway / Context Compression Proxy** in **TypeScript** on **Cloudflare Workers** (Hono + D1 + Upstash Redis). It sits transparently between OpenAI-compatible clients (OpenCode, Codex, etc.) and the real upstream AI API. The gateway optimizes what is sent **to** the main model and must never alter what comes **back**.
...
- Config via `.dev.vars`: upstream provider, memory AI, budget, auth, D1 binding.
- Health endpoint and basic app bootstrap.
- `wrangler dev` runs locally with D1 only (no Redis required).
