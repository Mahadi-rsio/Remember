...
describe("Conversation Isolation Integration (real SQLite DB)", () => {
  it("never leaks memories between conversations", async () => {
    ...
    const contentsA = await activeContents(db, "conv-a");
    const contentsB = await activeContents(db, "conv-b");
...
describe("Memory Merge & Staleness Integration (real SQLite DB)", () => {
  it("merges near-duplicate restatements instead of creating new items", async () => {
    ...
    await archive(db, [userMsg("Cloudisy uses PostgreSQL for its database", "d2")], conv);
    await archive(db, [userMsg("Cloudisy uses PostgreSQL for its database", "d3")], conv);
  ...
  it("only the latest ACTIVE value survives conflicting updates", async () => {
    ...
    await archive(db, [userMsg("Actually, I prefer light theme now", "s2")], conv);
  ...
  it("keeps unrelated memories alongside corrections", async () => {
    ...
    await archive(db, [userMsg("I prefer TypeScript for implementation", "u2")], conv);
    await archive(db, [userMsg("Actually, I prefer Rust now", "u3")], conv);
...
describe("Context Compiler Budget Integration (real SQLite DB)", () => {
  it("respects budget while preserving system and latest message with real memories", async () => {
    ...
    await archive(db, [userMsg("I prefer TypeScript for implementation", "b2")], conv);
    ...
    const compiled = await compileContext(db, messages, conv, {
      budget: 300,
      persistSnapshot: false,
    });
